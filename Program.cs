class Program
{
    static void Main(string[] args)
    {
        ...
        Ketrec oroszlánKetrec = new Ketrec("Oroszlán ketrec", állatok[0]);
        Ketrec zebraKetrec = new Ketrec("Zebra ketrec", állatok[1]);
        Ketrec krokodilKetrec = new Ketrec("Krokodil ketrec", állatok[2]);
        Ketrec elefántKetrec = new Ketrec("Elefánt ketrec", állatok[3]);
        ...
        for (int nap = 1; nap <= napokSzáma; nap++)
        {
            ...
            foreach (var ketrec in new List<Ketrec> { oroszlánKetrec, zebraKetrec, krokodilKetrec, elefántKetrec })
            {
                ...
                Console.WriteLine($"{ketrec.Állat.Név} ({ketrec.Állat.Fajta}) - {viewedCount} látogató nézte meg.");
            }
    ...
    {
        // Töröljük a konzolt és kiírjuk az üzenetet a kívánt színben
        Console.Clear();
